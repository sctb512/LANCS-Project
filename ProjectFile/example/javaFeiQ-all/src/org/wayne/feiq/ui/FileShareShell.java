...
public class FileShareShell extends Shell implements Listener {
	...
	public FileShareShell(Display display) {
...

		{
			...
			newColumnTableColumn_3.setText("修改时间");
		}
	...
	private void doGoInto() {
		...
		if (items != null && items.length > 0) {
			...
			if (currentDir.length() == 0) {
				...
				if (!items[0].getText(1).equals("文件夹")) {
	...
	void handleListFiles(FileResponse res) {
		...
		table.removeAll();
		if (sfiles == null) {
			MessageDialog.openWarning(this, "消息", res.getMessage());
			return;
		}
		lblStatus.setText(sfiles.size() + " 个对象");
